...
Irma alonso sánchez, versión en la que se garantiza la seguridad
"""
import time
import random
...
class Monitor():
    ...
    def are_no_car_s_ped(self): #True si pueden pasar los del norte
    	return (self.numcarspas.value==0 and self.numpedpas.value==0 )
    def are_no_car_n_ped(self):#True si pueden pasar los del sur
    	return (self.numcarnpas.value==0 and self.numpedpas.value==0 )
    def are_no_car_s_n(self):#True si pueden pasar los peatones
    	return (self.numcarspas.value==0 and self.numcarnpas.value==0)
